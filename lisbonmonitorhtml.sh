...
if [ $deviceOS == Android ]; then
  ...
  echo "<tr> <td> ${ai}"." </td> <td> $deviceStatus </td> <td> $deviceName </td> <td> ${deviceID} </td> <td> ${agentName:0:5} </td> <td> ${OSVersion} </td> <td> ${mitmCertsAvailable} </td> <td> ${deviceIPAddr} </td> <td> $httpResponse </td> <td> ${deviceWiFiName} </td> </tr>"
  ai=$((ai+1))
fi
fi
done
