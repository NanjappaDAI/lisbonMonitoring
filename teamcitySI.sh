...
siBuildDate=$(curl -s "http://192.168.1.213:8090/app/rest/buildTypes/id:Automation_AutomationSeleniumAgent_SeleniumIntegrationTest_RunTestsSeleniumIntegrationTestsMaster/builds/count:1" -H "Authorization: Bearer $accessKey" | sed -n 's:.*<startDate>\([0-9]\{8\}\).*<\/startDate>.*:\1:p')
...
echo "<tr> <td> "Selenium Integration" </td> <td> $passed_avg </td> <td> $failed_avg </td> </tr>" >> average.txt
echo "<tr> <td> "Selenium Integration" </td> <td> $siBuildDate </td> <td> $siStatus </td> </tr>" >> dailyStatus.txt